(function(){
  ...
  function refresh(){state.render('overview')}
  function decorate(){
    ...
    list.innerHTML=Object.values(state.accounts).map(function(a){
      var total=a.funds.reduce(function(s,f){return s+f.amount},0),daily=a.funds.reduce(function(s,f){return s+f.amount*f.today},0);
      if(!editing)return '<button class="account-card" data-account="'+esc(a.name)+'"><div><b>'+esc(a.name)+'</b><small>'+(a.funds.length?a.funds.length+' 项持仓':'暂无持仓')+'</small></div><div><strong>'+money(total)+'</strong><span class="'+(daily<0?'negative':'positive')+'">'+money(daily)+' 今日</span></div><span class="row-chevron">›</span></button>';
      return '<div class="account-card account-edit-row"><label class="account-check"><input type="checkbox" data-account-check="'+esc(a.name)+'" '+(selected.has(a.name)?'checked':'')+' aria-label="选择'+esc(a.name)+'"></label><div class="account-edit-main"><input class="account-name-input" data-account-name="'+esc(a.name)+'" value="'+esc(a.name)+'" aria-label="账户名称"><small>'+(a.funds.length?a.funds.length+' 项持仓':'暂无持仓')+'</small></div><div><strong>'+money(total)+'</strong><span class="'+(daily<0?'negative':'positive')+'">'+money(daily)+' 今日</span></div></div>';
    }).join('');
  }
